...
//IF RESPONSE IS NOT DEPENDENT ON KEY DELAY REPEAT RATE


/**********************
* Domain general code *
**********************/
//-this section declares functions for later use in the script
...
//function to compile data into form to save
...
function randn() {
	...
	//function to convert confidence rating into its position on the bar
	...
	* TASK-GENERAL CODE *
	********************/
	//-this section declares the structure of the entire experiment (that includes questionnaire items after the task)

	// Globals defined initially.
	var keydownfun = function() {};

	// Mutable global variables
	var datastring =""; // empty string to save task data
	var questDatastring =""; //empty string to save questionnaire data
	var payment="";
	var bonusPayFin=0; // participants are given a bonus up to 1USD the higher amount of points they score
	var set_cond = shuffle([1,2]); // set conditions for the task
	var is_training = true; // hasn't started practice trials yet
	var is_quiz = false; //is going to do the instruction quiz
	var q = -1; //this the counter for the questionnaires
	...
	*************/
	//BEGINING WITH INSTRUCTIONS FOR THE TASK AND THE TUTORIAL
	...
	//exceute three instruction pages
	...
	//this function starts the quitter code - i.e. will save incomplete data and warn about leaving the experiment
	...
	//this function starts the instruction quiz
	...
	//checking the answers of the instruction quiz to determine if they repeat the tutorial or move on to the task
	...
	//this is needed because if keys are on, then cannot type for the medlist questSection (one section in the psychiatric questionnaire)
	...
	//this is to execute the psychiatric questionnaire portion
	var questSection = function(){
		...
		// payment details needed - mTurk IDs and the amount of bonus pay they earned
		payment = payment.concat(assignmentId);
		payment = payment.concat("\t", workerId);
		...
		$.ajax("done", {
			type: "POST",
			async: false,
			data:  {workerId: workerId,assignmentId:assignmentId, hitId:hitId, datastring: datastring, payment:payment, when:endTime},
		});
		return false;
	...
	************************/
	//--this is to execute the instruction quiz
	...
	var Instructions = function( screens ) {
		...
		//if the next page is the quiz, run the quiz script
		...
		//when its time to start the task experiment, start new experimental phase
	...
	* CODE FOR TEST     *
	********************/
	// this is the code pertaining to the experimental TASK
	...
	var ExptPhase = function() {
		...
		// Stimulus variables
		var stim_std            = 12; //standard deviation for particle landing
		...
		var stim_pracHazRate	  = 0.01; //hazard rate for the practice phase
		var stim_cirDia         = 500; // diameter of big circle (every degree = 4 px)
		var stim_cirThickness   = 15; //thickness of circle
		...
		var stim_bucketDepth    = 50; //thickness of bucket
		var stim_bucketDia      = stim_cirDia + stim_bucketDepth; //diameter of bucket
		...
		var stim_moneyDia       = 20; //diameter of coin
		...
		var  conf_checkTrial      = 20; //how many trials before checking how many they kept choosing the same confidence
		var  conf_checkTrial2     = 50; //second check of their confidence levels at trial 50

		// THIS IS TO CALCULATE CPP AND PRESET PARTICLE LOCATION (IN DEGREES) FOR STABLE AND VOLATILE BLOCKS
		var nCond = set_cond.length;
		var nTrialsPerCond = 150;//150
		var nTrialsPrac = 10;//10
		var nTrialsTotal = nTrialsPerCond*nCond;
		var nBlocks = 5;//4
		...
		kd.SPACE.press(function () {
			...
			var draw_conf = function(){
				...
				//draw particles flying to the edge
				var draw_dot = function(){
					...
					}else{ //for when its practice phase, it is the most stable lol
					...
					//draw the particle shoot motion
					draw_dotShoot(canvas_xCenter, canvas_yCenter, stim_coord, stim_cirDia, stim_dotDia, col_money, time_dotSpeed, dot_pos);

					//draw what happens after particle lands on circle edge
					setTimeout(function() {

						//if it is a hit, the bucket turns green
						if (data_hit===true) {
							...
							//if it is a miss, the bucket turns red
							paper.circle(canvas_xCenter, canvas_yCenter, stim_dotDia / 2).attr({
								stroke: col_red,
								fill: col_red  });
         ...
         //function to reset the experiment if participants keep choosing the same confidence level over 20 or 50 trials
                ...
                //experiment is broken up into 4 blocks, this allows a break screen in between blocks
              ...
              //function to draw each trial
								var draw_trial = function() {
									// console.log("Drawing trial - practice: " +data_nTrialPrac+ " or actual: " +data_nTrial);
									// buck_done = 0;
									buckListening = true; //lets the keyboard listen to move the bucket
									confListening = false; //lets the keyobard listen to move the confidence bar
               ...
               //fuunction to start the experimental task
